;(function($){
	var methods = {
		...
		pageData: function(_v, _current){
            ...
            var t = opts.totalData, p = opts.pageData, ajaxOpts = null, postData = opts.postData
			postData.page = _current
			if(opts.ajaxSetData&&(typeof(opts.ajaxSetData)==='boolean')){
				if(opts.url!=='' && typeof(opts.url)==='string'){
					ajaxOpts = methods.ajaxData(opts.url, postData);
			...
			if(t%p === 0){
                ...
                opts.pageStep = parseInt(t/p);
            }else{
                opts.pageCount = parseInt(t/p)+1;
                opts.pageStep = parseInt(t/p)+1;
            }
            if(opts.pageCount>0){
            	_v.data("options",opts);
		...
		ajaxData: function(_url, _current){
			 ...
			 */
            // console.log($.fn.zPager.defaults, 'zPager')
				...
				$.ajax({
	                ...
	                success: function(data) {
	                	if(data.data.total && (data.data.total!==0)){
	                        ...
	                        parms['rows'] = data.data.data;
	                        parms['pageCount'] = data.data.last_page;
	                	}else{
	                		$.pageError(3);
                            $('#wraper').empty()
	                	}
